...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Copy Multi Neural Network model (and possibly change binary/text format)\n"
        "Usage:  multi-nnet-copy [options] <model-in> <model-out>\n"
        "e.g.:\n"
    ...
    if(remove_first_layers > 0) {
      for(int32 i=0; i<remove_first_layers; i++) {
        multi_nnet.RemoveSharedComponent(0);
      }
    }
