...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Add softmax layer to MultiNnet shared layers\n"
        "Usage:  multi-nnet-add-softmax [options] <multi-nnet-in> <multi-nnet-out>\n"
        "e.g.:\n"
        " multi-nnet-add-softmax --binary=false multi_nnet.mdl multi_nnet_new.mdl\n";
    ...
    std::string multi_nnet_out_filename = po.GetArg(2);
