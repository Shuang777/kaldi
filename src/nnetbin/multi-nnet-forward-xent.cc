...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      //download from GPU
      obj_diff_host.resize(obj_diff.size());
      for (int32 i=0; i< obj_diff.size(); i++) {
        obj_diff_host[i].Resize(obj_diff[i]->NumRows(), obj_diff[i]->NumCols());
        obj_diff[i]->CopyToMat(&obj_diff_host[i]);
      }
      
      //check for NaN/inf
      for (int32 i=0; i< obj_diff_host.size(); i++) {
        for (int32 r = 0; r < obj_diff_host[i].NumRows(); r++) {
          for (int32 c = 0; c < obj_diff_host[i].NumCols(); c++) {
            BaseFloat val = obj_diff_host[i](r,c);
      ...
      // write
      for (int32 i=0; i< obj_diff_host.size(); i++) {
        feature_writer.Write(feature_reader.Key(), obj_diff_host[i]);
      }
