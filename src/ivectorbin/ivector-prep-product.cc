...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "\n"
        "Usage: ivector-prep-product [options] <utt-pairs> <ivector-rspecifier> <feat-wspecifier> <ali-wspecifier>\n"
        " e.g.: ivector-prep-product data/utt-pairs scp:exp/ivectors.scp scp,ark:exp/ivector-pairs.scp,exp/ivector-pairs.ark scp,ark:exp/ivector-pairs-ali.scp,exp/ivector-pairs-ali.ark\n";
    ...
    const bool single_frame = (frames_per_utt == 1);
    std::string utt_id;
    ...
    while (std::getline(ki.Stream(), line)) {
      ...
      if (num_done == 0) {
        features.Resize(frames_per_utt, ivector1.Dim());
      }
      SubVector<BaseFloat> ivec(features, num_frames);
      ivec.CopyFromVec(ivector1);
      const Vector<BaseFloat> &ivector2 = ivector_reader.Value(key2);
      ivec.MulElements(ivector2);

      num_frames++;
      num_done++;
      
      std::stringstream ss;  ss << num_utts;
      utt_id = single_frame ? key1 + "-"  + key2 : utt_prefix + "utt_" + ss.str();
      if (num_frames == frames_per_utt) {
        kaldi_writer.Write(utt_id, features);
        alignment_writer.Write(utt_id, alignments);
        num_frames = 0;
        num_utts++;
      }
    }
