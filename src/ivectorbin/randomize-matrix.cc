...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage: randomize-matrix [options] <matrix-in-rspecifier> <matrix-out-wspecifier>\n"
    ...
    if (!in_is_rspecifier) {
      ...
      feature_randomizer.AddData(mat);
      ...
      for ( ; !feature_randomizer.Done(); feature_randomizer.Next()) {
        const Matrix<BaseFloat> mat_cv = feature_randomizer.Value();
        const Matrix<BaseFloat> mat_left = feature_randomizer.LeftOverValue();

        Output ko(matrix_out_fn, binary);
        mat_cv.Write(ko.Stream(), binary);
        mat_left.Write(ko.Stream(), binary);
      }
