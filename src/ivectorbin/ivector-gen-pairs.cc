...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "\n"
        "Usage: ivector-gen-pairs [options] <utt-pairs> <ivector-rspecifier> <feat-wspecifier> <ali-wspecifier>\n"
        " e.g.: ivector-gen-pairs data/utt-pairs scp:exp/ivectors.scp scp,ark:exp/ivector-pairs.scp,exp/ivector-pairs.ark scp,ark:exp/ivector-pairs-ali.scp,exp/ivector-pairs-ali.ark\n";
    
    ParseOptions po(usage);
    int32 num_frames_per_utt = 1024;
    po.Register("num-frames-per-utt", &num_frames_per_utt, "Number of pairs per utterance for nnet training (default is 1024)");
    ...
    std::vector<int32> alignments(num_frames_per_utt);
    ...
    while (std::getline(ki.Stream(), line)) {
      ...
      if (num_done == 0) {
        features.Resize(num_frames_per_utt, 2*ivector1.Dim());
      }
      ...
      num_frames++;
      num_done++;
      if (num_frames == num_frames_per_utt) {
        std::stringstream ss;  ss << num_utts;
        kaldi_writer.Write("utt_" + ss.str(), features);
        alignment_writer.Write("utt_" + ss.str(), alignments);
        num_frames = 0;
        num_utts++;
      }
    }
