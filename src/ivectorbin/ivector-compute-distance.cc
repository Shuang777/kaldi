...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Computes distance between iVectors; useful in application of an\n"
        ...
        "\n"
        "Usage:  ivector-compute-distance [options] <trials-in> "
        "<ivector1-rspecifier> <ivector2-rspecifier> <scores-out>\n"
        "e.g.: \n"
        " ivector-compute-distance trials ark:train_ivectors.scp ark:test_ivectors.scp trials.scored\n"
    ...
    while (std::getline(ki.Stream(), line)) {
      ...
      // best to just crash.
      Vector<BaseFloat> diff = ivector1;
      diff.AddVec(-1, ivector2);
      BaseFloat distance = diff.Norm(2);
      sum += distance;
      sumsq += distance * distance;
      num_done++;
      ko.Stream() << key1 << ' ' << key2 << ' ' << distance << std::endl;
    }
    
    if (num_done != 0) {
      BaseFloat mean = sum / num_done, scatter = sumsq / num_done,
          variance = scatter - mean * mean, stddev = sqrt(variance);
      KALDI_LOG << "Mean distance was " << mean << ", standard deviation was "
                << stddev;
    }
