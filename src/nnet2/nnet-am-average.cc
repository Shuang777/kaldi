...
int main(int argc, char *argv[]) {
  try {
    ...
    bool sum = false;
    int num_masters = -1;
    int num_layers_master = -1;
    
    ParseOptions po(usage);
    po.Register("sum", &sum, "If true, sums instead of averages.");
    po.Register("binary", &binary_write, "Write output in binary mode");
    po.Register("num-masters", &num_masters, "Set number of master nnets for average");
    po.Register("num-layers-master", &num_layers_master, "Set number of layers to average for master nnets (usually top few layers)");
    ...
    if (num_masters == -1)
      num_masters = num_inputs;
    KALDI_ASSERT(num_masters <= num_inputs);

    BaseFloat scale_master = (sum ? 1.0 : 1.0 / num_masters);

    if (num_layers_master == -1)
      am_nnet1.GetNnet().Scale(scale);
    else
      am_nnet1.GetNnet().ScaleMaster(scale, scale_master, num_layers_master);
    ...
    for (int32 i = 2; i <= num_inputs; i++) {
      ...
      if (num_layers_master == -1)
        ...
        if (i <= num_masters)
          am_nnet1.GetNnet().AddNnetMaster(scale, scale_master, num_layers_master, am_nnet.GetNnet());
        else
          am_nnet1.GetNnet().AddNnetMaster(scale, 0, num_layers_master, am_nnet.GetNnet());
      }
    }
