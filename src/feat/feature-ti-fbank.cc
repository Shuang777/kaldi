...
#include "feat/feature-ti-fbank.h"


namespace kaldi {

TiFbank::TiFbank(const TiFbankOptions &opts)
...
const TiMelBanks *TiFbank::GetTiMelBanks(BaseFloat vtln_warp) {
...
void TiFbank::Compute(const VectorBase<BaseFloat> &wave,
                    BaseFloat vtln_warp,
...
void TiFbank::ComputeInternal(const VectorBase<BaseFloat> &wave,
  ...
  int32 cols_out = opts_.num_bins + opts_.use_energy;
  if (rows_out == 0) {
    output->Resize(0, 0);
    *wave_remainder = wave;
    return;
  ...
  for (int32 r = 0; r < rows_out; r++) {
    ...
    if (opts_.htk_compat && opts_.use_energy) {
      ...
      for (int32 i = 0; i < opts_.num_bins; i++) {
        this_output(i) = this_output(i+1);
      }
      this_output(opts_.num_bins) = energy;
    }
  }
}
