...
void MatrixRandomizer::AddData(const Matrix<BaseFloat>& m) {
...
void MatrixRandomizer::Randomize(const std::vector<int32>& mask) {
  ...
  for(int32 i=0; i<mask.size(); i++) {
    data_.Row(i).CopyFromVec(data_aux_.Row(mask[i]));
  }
...
const Matrix<BaseFloat>& MatrixRandomizer::Value() {
...
const Matrix<BaseFloat>& MatrixRandomizer::LeftOverValue() {
