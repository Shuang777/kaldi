...
for condition in short3; do # could also add "10sec" and "summed" here.
  ...
  for disk in {1..9}; do
    sph_src=$src/2008_nist_sre_te_d$disk/data/test/data/$condition
    if [ ! -d $sph_src ]; then
      continue
    fi
  ...
  cat $src/2008_nist_sre_te_d9/data/trials/*-${condition}.ndx  | sed 's/.sph:/_/' | awk '{print $3, $2}' | sort | uniq > $data/spk2gender
...
tail -n +2 $src/2008_nist_sre_te_d9/data/keys/NIST_SRE08_KEYS.v0.1/trial-keys/NIST_SRE08_short2-short3.trial.key | \
