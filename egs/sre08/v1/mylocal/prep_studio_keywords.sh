...
for i in `ls $datadir`; do
  ...
  cat $thisdatadir/gender | sort > $thisdir/spk2gender
  find $thisdatadir -name "*.wav" | perl -e 'while ($line = <>) { $line =~ s/\s+$//; $line =~ m/.*\/(.*[0-9]*).wav/; $utt=$1; printf("%s %s\n", $utt, $line);}' | sort > $thisdir/wav.scp
  find $thisdatadir -name "*.wav" | perl -e 'while ($line = <>) { $line =~ s/\s+$//; $line =~ m/.*\/(.*[0-9]*).wav/; $utt=$1; $utt =~ m/([^_0-9]*)(_[0-9]*)*/; $spk=$1; printf("%s %s\n", $utt, $spk);}' | sort -u > $thisdir/utt2spk
