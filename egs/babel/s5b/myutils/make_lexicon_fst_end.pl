...
if (@ARGV != 1 && @ARGV != 4 && @ARGV != 5) {
  print STDERR
    "Usage: make_lexicon_fst.pl [--pron-probs] lexicon.txt [silprob silphone silword [sil_disambig_sym]] >lexiconfst.txt
...
if (@ARGV == 0) {
  ...
  ($silprob,$silphone,$silword) = @ARGV;
} else {
  ($silprob,$silphone,$silword,$sildisambig) = @ARGV;
}
if ($silprob != 0.0) {
...
if ( $silprob == 0.0 ) { # No optional silences: just have one (loop+final) state which is numbered zero.
  ...
  if (!defined $sildisambig) {
    print "$startstate\t$loopstate\t$silphone\t$silword\t$silcost\n"; # silence.
    print "$silstate\t$loopstate\t$silphone\t$silword\n";             # no cost.
#    print "$loopstate\t$loopstate\t#0\t#0\n"; # add self loop, no cost
    $nextstate = 3;
  } else {
    $disambigstate = 3;
    $nextstate = 4;
    print "$startstate\t$disambigstate\t$silphone\t$silword\t$silcost\n"; # silence.
    print "$silstate\t$disambigstate\t$silphone\t$silword\n"; # no cost.
...
#    print "$loopstate\t$loopstate\t#0\t#0\n"; # add self loop, no cost
  ...
  while (<L>) {
    ...
    while (@A > 0) {
      ...
      if (@A > 0) {
        $ns = $nextstate++;
        if (@A == 1 and $A[0] =~ '#') {   # print out the word before disambig symbol
          $output = $word_or_eps;
          $word_or_eps = $eps;
        } else {
          $output = $eps;
        }
        print "$s\t$ns\t$p\t$output$pron_cost_string\n";
