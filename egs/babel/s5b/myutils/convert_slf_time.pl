...
$usage="Convert kaldi lattices to HTK SLF (v1.1) format.\n".
       ...
       "  --time-mode     This is kaldi-lattice with end time instead of alignments\n".
       "\n";
...
while (@ARGV gt 0 and $ARGV[0] =~ m/^--/) {
  ...
  elsif ($param eq "--time-mode") { $timemode = 1;}
  else {
...
while(<FI>) {
  ...
  if (@A == 1 and $utt eq "") {
    ...
    $utt = $A[0];
    $nodes_map{0} = 0;
    $nodes{$nodes_map{0}} = { W=>"!NULL", t=>0.0, n_out_arcs=>0 }; #initial node
  } elsif (@A == 1) {
    # accepting node without FST weight, store data for link to terminal super-state
    $s = $A[0];
    if (defined $nodes_map_weight{$s}) {
      $as = $nodes_map_weight{$s}{a};
      $gs = $nodes_map_weight{$s}{l};
    }
    $accepting_states{$nodes_map{$s}} = { W=>"!NULL", v=>0, a=>$as, l=>$gs };
    ...
    if ( $timemode == 0 ) {
      ...
      $ss = scalar ((split(/_/, $ss))[-1]);     # select last match
      if ( $ss == '' ) {
        if (defined $nodes_map_weight{$s}) {
          $ss = $nodes_map_weight{$s}{t};
        } else {
          $ss = $nodes{$nodes_map{$s}}{t};
        }
      }
    }
    
    # update the end time
    die "Node $s not yet visited, is lattice sorted topologically? $utt" unless exists $nodes_map{$s};
    if ( $timemode == 0 ) {
      $time_end = $nodes{$nodes_map{$s}}{t} + $ss;
    } else {
      $time_end = $ss;
    }
