...
function filter_recordings {
  ...
  # recording-ids.
    ...
    if [ ! -f $data/wav.scp ]; then
      echo "$0: $data/wav.scp does not exists"
      exit 1;
    fi
...
function filter_speakers {
  ...
  utils/utt2spk_to_spk2utt.pl $data/utt2spk | sort -u > $data/spk2utt
  ...
  utils/spk2utt_to_utt2spk.pl $data/spk2utt | sort -u > $data/utt2spk
