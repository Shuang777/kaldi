...
if [ $stage -le 3 ]; then
  $cmd $mem_req JOB=1:$nj $dir/log/vecs_pass1.JOB.log \
...
if [ $stage -le 4 ]; then
  $cmd $mem_req JOB=1:$nj $dir/log/vecs_pass2.JOB.log \
    gunzip -c $dir/pre_lat.JOB.gz \| \
...
if $use_fmllr; then
  ...
  if [ $stage -le 5 ]; then # compute fMLLR transforms.
    ...
    $cmd $mem_req JOB=1:$nj $dir/log/fmllr.JOB.log \
      gunzip -c $dir/pre_lat.JOB.gz \| \
...
if [ $stage -le 6 ]; then
  $cmd $parallel_opts JOB=1:$nj $dir/log/rescore.JOB.log \
    ...
    lattice-determinize-pruned$thread_string --acoustic-scale=$acwt --beam=$lattice_beam ark:- ark:- \| \
    $aligncmd $srcdir/final.mdl ark:- \
