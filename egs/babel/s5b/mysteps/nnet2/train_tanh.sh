...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  [ ! -z "$utts_subset_factor" ] && extra_opts+=(--utts-subset-factor $utts_subset_factor)
  mysteps/nnet2/get_egs.sh $egs_opts "${extra_opts[@]}" \
      --samples-per-iter $samples_per_iter \
      --num-jobs-nnet $num_jobs_nnet --stage $get_egs_stage \
      --cmd "$cmd" $egs_opts --io-opts "$io_opts" \
      $data $lang $alidir $dir || exit 1;
  if [ ! -z $semidata ]; then
    num_jobs_nnet_semi=$(echo "scale=0;$num_jobs_nnet*$semirate/1" | bc)
    mysteps/nnet2/get_egs.sh $egs_opts "${extra_opts[@]}" \
        --samples-per-iter $samples_per_iter \
        --num-jobs-nnet $num_jobs_nnet_semi --stage $get_egs_stage \
        --cmd "$cmd" $egs_opts --io-opts "$io_opts" \
        --egsdir egs_semi --num-utts-subset 0 \
        --listext ".semi" \
        --transform-dir $semitransdir \
        $semidata $lang $semialidir $dir
  fi
fi

[ -z $egs_dir ] && egs_dir=$dir/egs
[ ! -z $semidata ] && egs_semi_dir=$dir/egs_semi

iters_per_epoch=`cat $egs_dir/iters_per_epoch`  || exit 1;
[ ! -z "$egs_semi_dir" ] && iters_per_epoch_semi=`cat $egs_semi_dir/iters_per_epoch`
...
while [ $x -lt $num_iters ]; do
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    nnet-am-info "$mdl" > $dir/foo  2>/dev/null || exit 1

    if [ ! -z $semidata ]; then
      lr_string=$(grep -o 'learning-rate=[0-9.]*' $dir/foo | sed -e 's#learning-rate=##g' -e '$ d' | sed -e ':a;N;$!ba;s/\n/:/g')
      lr_string="$lr_string:0"
      nnet-am-copy --learning-rates=$lr_string "$mdl" $dir/semi.mdl

      $cmd $parallel_opts JOB=1:$num_jobs_nnet_semi $dir/log/train.semi.$x.JOB.log \
        nnet-shuffle-egs --buffer-size=$shuffle_buffer_size --srand=$x \
        ark:$egs_semi_dir/egs.JOB.$[$x%$iters_per_epoch_semi].ark ark:- \| \
        nnet-train$train_suffix \
           --minibatch-size=$minibatch_size --srand=$x $dir/semi.mdl \
          ark:- $dir/$[$x+1].semi.JOB.mdl &
      
      for n in `seq 1 $num_jobs_nnet_semi`; do
        nnets_list="$nnets_list $dir/$[$x+1].semi.$n.mdl"
      done
    fi
    ...
    if [ ! -z $semidata ]; then
      ...
      num_all_components=`grep component $dir/foo | tail -1 | awk '{print $2}'`
      top_affine_components=`grep Affine $dir/foo | tail -1 | awk '{print $2}'`
      num_layers_master=$(($num_all_components - $top_affine_components + 1))
      $cmd $dir/log/average.$x.log \
        nnet-am-average --num-masters=$num_jobs_nnet --num-layers-master=$num_layers_master $nnets_list - \| \
        nnet-am-copy --learning-rates=$lr_string - $dir/$[$x+1].mdl || exit 1;
    fi
