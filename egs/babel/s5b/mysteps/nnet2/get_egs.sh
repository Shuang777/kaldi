...
utts_subset_factor=    # percent of utterances in validation and training
...
if [ ! -z $utts_subset_factor ]; then
  num_utts=$(wc $data/utt2spk | awk '{print $1}')
  num_utts_subset=$(echo "scale=0;$utts_subset_factor*$num_utts/1" | bc)
  echo "num_utts_subset set to $num_utts_subset"
fi

# Get list of validation utterances. 
awk '{print $1}' $data/utt2spk | utils/shuffle_list.pl | head -$num_utts_subset \
    > $dir/valid_uttlist$listext || exit 1;
...
awk '{print $1}' $data/utt2spk | utils/filter_scp.pl --exclude $dir/valid_uttlist$listext | \
     head -$num_utts_subset > $dir/train_subset_uttlist$listext || exit 1;
...
case $feat_type in
  raw) feats="ark,s,cs:utils/filter_scp.pl --exclude $dir/valid_uttlist$listext $sdata/JOB/feats.scp | apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
    valid_feats="ark,s,cs:utils/filter_scp.pl $dir/valid_uttlist$listext $data/feats.scp | apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
    train_subset_feats="ark,s,cs:utils/filter_scp.pl $dir/train_subset_uttlist$listext $data/feats.scp | apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
   ;;
  lda) 
    splice_opts=`cat $alidir/splice_opts 2>/dev/null`
    cp $alidir/splice_opts $dir 2>/dev/null
    cp $alidir/final.mat $dir    
    feats="ark,s,cs:utils/filter_scp.pl --exclude $dir/valid_uttlist$listext $sdata/JOB/feats.scp | apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
      valid_feats="ark,s,cs:utils/filter_scp.pl $dir/valid_uttlist$listext $data/feats.scp | apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
      train_subset_feats="ark,s,cs:utils/filter_scp.pl $dir/train_subset_uttlist$listext $data/feats.scp | apply-cmvn --norm-vars=$norm_vars --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    ;;
...
if [ $stage -le 0 ]; then
  ...
  echo $num_frames > $dir/num_frames$listext
else
  num_frames=`cat $dir/num_frames$listext` || exit 1;
fi
...
for x in `seq 1 $num_jobs_nnet`; do
  for y in `seq 0 $[$iters_per_epoch-1]`; do
    utils/create_data_link.pl $dir/$egsdir/egs.$x.$y.ark
    utils/create_data_link.pl $dir/$egsdir/egs_tmp.$x.$y.ark
  done
  for y in `seq 1 $nj`; do
    utils/create_data_link.pl $dir/$egsdir/egs_orig.$x.$y.ark
  done
done
...
if [ $stage -le 2 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    nnet-get-egs $nnet_context_opts "${spk_vecs_opt[@]}" "$valid_feats" \
     ...
     "ark:$dir/$egsdir/valid_all.egs" || touch $dir/.error &
  $cmd $dir/log/create_train_subset$listext.log \
    nnet-get-egs $nnet_context_opts "${spk_vecs_opt[@]}" "$train_subset_feats" \
     "ark,cs:gunzip -c $alidir/ali.*.gz | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- |" \
     "ark:$dir/$egsdir/train_subset_all.egs" || touch $dir/.error &
  wait;
  [ -f $dir/.error ] && exit 1;
  echo "Getting subsets of validation examples for diagnostics and combination."
  $cmd $dir/log/create_valid_subset_combine.log \
    nnet-subset-egs --n=$num_valid_frames_combine ark:$dir/$egsdir/valid_all.egs \
        ark:$dir/$egsdir/valid_combine.egs || touch $dir/.error &
  $cmd $dir/log/create_valid_subset_diagnostic.log \
    nnet-subset-egs --n=$num_frames_diagnostic ark:$dir/$egsdir/valid_all.egs \
    ark:$dir/$egsdir/valid_diagnostic.egs || touch $dir/.error &

  $cmd $dir/log/create_train_subset_combine.log \
    nnet-subset-egs --n=$num_train_frames_combine ark:$dir/$egsdir/train_subset_all.egs \
    ark:$dir/$egsdir/train_combine.egs || touch $dir/.error &
  $cmd $dir/log/create_train_subset_diagnostic.log \
    nnet-subset-egs --n=$num_frames_diagnostic ark:$dir/$egsdir/train_subset_all.egs \
    ark:$dir/$egsdir/train_diagnostic.egs || touch $dir/.error &
  wait
  cat $dir/$egsdir/valid_combine.egs $dir/$egsdir/train_combine.egs > $dir/$egsdir/combine.egs

  for f in $dir/$egsdir/{combine,train_diagnostic}.egs; do
    [ ! -s $f ] && echo "No examples in file $f";
  done
  rm $dir/$egsdir/valid_all.egs $dir/$egsdir/train_subset_all.egs $dir/$egsdir/{train,valid}_combine.egs
  fi
fi
...
if [ $stage -le 3 ]; then
  ...
  echo $num_jobs_nnet >$dir/$egsdir/num_jobs_nnet
  echo $iters_per_epoch >$dir/$egsdir/iters_per_epoch
  echo $samples_per_iter_real >$dir/$egsdir/samples_per_iter
  ...
  for n in `seq 1 $num_jobs_nnet`; do
    egs_list="$egs_list ark:$dir/$egsdir/egs_orig.$n.JOB.ark"
  done
...
if [ $stage -le 4 ]; then
  ...
  if [ $iters_per_epoch -eq 1 ]; then
    ...
    for n in `seq 1 $num_jobs_nnet`; do
      cat $dir/$egsdir/egs_orig.$n.*.ark > $dir/$egsdir/egs_tmp.$n.0.ark || exit 1;
      rm $dir/$egsdir/egs_orig.$n.*.ark  # don't "|| exit 1", due to NFS bugs...
    done
  else # We'll have to split it up using nnet-copy-egs.
    egs_list=
    for n in `seq 0 $[$iters_per_epoch-1]`; do
      egs_list="$egs_list ark:$dir/$egsdir/egs_tmp.JOB.$n.ark"
    done
    ...
    $cmd $io_opts JOB=1:$num_jobs_nnet $dir/log/split_egs.JOB.log \
      nnet-copy-egs --random=$random_copy --srand=JOB \
        "ark:cat $dir/$egsdir/egs_orig.JOB.*.ark|" $egs_list '&&' \
        '(' rm $dir/$egsdir/egs_orig.JOB.*.ark '||' true ')' || exit 1;
  fi
fi
...
if [ $stage -le 5 ]; then
  ...
  for n in `seq 0 $[$iters_per_epoch-1]`; do
    $cmd $io_opts JOB=1:$num_jobs_nnet $dir/log/shuffle.$n.JOB.log \
      ...
      ark:$dir/$egsdir/egs_tmp.JOB.$n.ark ark:$dir/$egsdir/egs.JOB.$n.ark '&&' \
      '(' rm $dir/$egsdir/egs_tmp.JOB.$n.ark '||' true ')' || exit 1;
  done
fi
