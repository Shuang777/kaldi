...
words=words.txt
latname="lat"
[ $wrd2syl == "true" ] && words=words.merge.txt && latname='wrdlat'

if [ ! -f $decodedir/.done.align ]; then
  for f in $word_boundary; do
    [ ! -f $f ] && echo "make_index.sh: no such file $f" && exit 1;
  done
  $cmd JOB=1:$nj $decodedir/log/align.JOB.log \
    mv $decodedir/${latname}.JOB.gz $decodedir/${latname}.JOB.gz.bak '&&' \
    lattice-copy "ark:gunzip -c $decodedir/${latname}.JOB.gz.bak|" ark:- \| \
    lattice-align-words $silence_opt --max-expand=$max_expand $word_boundary $model ark:- \
    "ark:|gzip -c > $decodedir/${latname}.JOB.gz" '&&' \
    rm $decodedir/${latname}.JOB.gz.bak
  touch $decodedir/.done.align
fi

$cmd JOB=1:$nj $kwsdir/log/index.JOB.log \
  lattice-add-penalty --word-ins-penalty=$word_ins_penalty "ark:gzip -cdf $decodedir/${latname}.JOB.gz|" ark:- \| \
