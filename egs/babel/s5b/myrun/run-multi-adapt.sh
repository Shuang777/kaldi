...
multidir=/u/drspeech/data/swordfish/users/suhang/projects/swordfish/kaldi/kaldi-effort/expsnew/multillp1/exp/train_fmllr_dnn
dnndir=exp/${traindata}_tri9_multidnn_adapt
[ $multi_split != 0 ] && dnndir=${dnndir}_split${multi_split}
[ $semi == true ] && dnndir=${dnndir}_semi
if [ $stage -le 1 ]; then
if [ ! -f $dnndir/.done ]; then
  echo "-----------------------------------------------------------------"
  echo "Begin adapting dnn in $dnndir on" `date`
  echo "-----------------------------------------------------------------"
  [ ! -d $dnndir ] && mkdir -p $dnndir
  if [ $semi == false ]; then
    $cuda_cmd $dnndir/train_nnet.log \
    mysteps/train_nnet.sh --feature-transform $multidir/final.feature_transform --mlp-init $multidir/$nnet --learn-rate 0.0008 --cv-subset-factor 0.1 data/${traindata} data/lang exp/${traindata}_tri5_ali $dnndir
  else
    $cuda_cmd $dnndir/train_nnet.log #\
