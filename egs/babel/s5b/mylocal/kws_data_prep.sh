...
if [ -z $word_surface ]; then
  ...
  [ ! -z $silence_word ] && silence_opt="--silence_word $silence_word" || silence_opt=
  cat $word_surface | hescii.py -d |\
  mylocal/word_surf_to_kw_surf.pl $kwsdatadir/keywords.txt - - |\
    mylocal/kw_surface_to_fst.pl $kwsdatadir/keywords.txt $kwsdatadir/words.txt - $kwsdatadir/keywords.fsts
fi
