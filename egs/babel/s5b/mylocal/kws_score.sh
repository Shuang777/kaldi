...
help_message="$0: score the kwslist using the F4DE scorer from NIST
  Example:
    ...
    --kwsext <kwsext> #allows for kws extension
...
for file in $kwsdatadir/scoring.ecf.xml $kwsdatadir/rttm $kwlist ; do
    if [[ ! -f "$file" ]] ; then
...
echo KWSEval -e $kwsdatadir/scoring.ecf.xml -r $kwsdatadir/rttm -t $kwlist \
    -s $kwsoutputdir/kwslist.xml -w -c -o -b -O -B -d -f $kwsoutputdir

KWSEval -e $kwsdatadir/scoring.ecf.xml -r $kwsdatadir/rttm -t $kwlist \
    -s $kwsoutputdir/kwslist.xml -w -c -o -b -O -B -d -f ${kwsoutputdir}${f4de_prefix} || exit 1;
