...
#     sys.exit(1)
    ...
    format_str = r"%07d"    # currently we set this to keep compatible with HTK format
...
def main():
  ...
  parser.usage=':'.join(parser.format_usage().split(':')[1:]) \
      + 'e.g. :  cat pred.1 | %(prog)s exp/tri4b_whole_resegment_dev10h/phone_map.txt data/dev10h.seg/segments'
  ...
  temp_dir = os.path.dirname(options.phone_map) + "/rttm_classes"
  ...
  for line in sys.stdin:
    f,phone_seq = line.strip().split(' ',1)
    A = phone_seq.split()
