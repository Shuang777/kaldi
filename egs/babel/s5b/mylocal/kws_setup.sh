...
allowed switches:
      --kwsext                           # the extension for non-standard kws
      --kwsdatadir /path/to/outputdir    # default = <data-dir>/kws[kwsext]
      ...
      --word-surface                     # The file with the surface forms for single-words
              "
...
chmod 644 $kwsdatadir/rttm

[ ! -z $word_surface ] && word_surface_opt="--word_surface $word_surface" || word_surface_opt=
[ ! -z $silence_word ] && sil_opt="--silence-word $silence_word" || sil_opt=

mylocal/kws_data_prep.sh --case-insensitive ${case_insensitive} $word_surface_opt \
