...
# Check some files.
for f in $data_sup/feats.scp $data_unsup/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree $latdir/ali.1.gz $latdir/weights.1.gz $latdir/../final.mdl $latdir/../tree; do
...
if [ $stage -le 3 ]; then
  ...
  $cmd $io_opts JOB=1:$nj_unsup $dir/log/get_weighted_egs.JOB.log \
    ...
    "ark,s,cs:gunzip -c $latdir/ali.JOB.gz | convert-ali $latdir/../final.mdl $alidir/final.mdl $dir/tree ark:- ark:- | ali-to-pdf $alidir/final.mdl ark:- ark:- | ali-to-post ark:- ark:- |" \
