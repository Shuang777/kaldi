...
GetOptions("fragmentMarkers=s" => \$fragMarkers,
           ...
           "ignore-transcripts=s" => \$ignore_transcripts
           );
...
if ($#ARGV == 1) {
  ...
  print STDERR ("\t--ignore-transcripts (true|false) Do not process transcriptions\n");
  exit(1);
}
...
if ($ignore_transcripts eq "true"){
  printf STDERR ("$0: Ignoring transcription files in $TranscriptionDir\n");
} elsif (-d $TranscriptionDir) {
  ...
  if ($#TranscriptionFiles >= 0) {
    ...
    while ($filename = shift @TranscriptionFiles) {
      ...
      while ($line=<TRANSCRIPT>) {
        chomp $line;
        if ($line =~ m:^\[([0-9]+\.*[0-9]*)\]$:) {    # time info line
          $thisTimeMark = $1;
          ...
          ##################################################
          $utteranceID =  $fileID;
          $utteranceID .= sprintf ("_%07i_%07i", (100*$prevTimeMark), (100*$thisTimeMark));
          ##################################################
          ...
          }   # loop over words in transcript line end
      ...
      } # loop over lines in transcript end
      close(TRANSCRIPTION);
...
if ($#AudioFiles >= 0) {
  ...
  while ($filename = shift @AudioFiles) {
    ...
    if (exists $numUtterancesInFile{$fileID} or $ignore_transcripts eq "true") {
      ...
      if ($ignore_transcripts eq "true") {
        $transcription{$fileID} = 1;
        $speakerID{$fileID} = $fileID;
        $baseFileID{$fileID} = $fileID;
        $numUtterances++;
      }
    } else {
...
if ($numUtterances>0) {
  ...
  sqrt(($totalSpeechSq/$numUtterances)-($totalSpeech*$totalSpeech)), $numWords/$numUtterances);
}
