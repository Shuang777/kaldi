...
if [ $# != 3 ]; then
  echo "Usage: $0 /path/to/SWBD <data-dir> <data-local-dir>"
  echo " e.g.: $0 /u/drspeech/data/SWB1r2 data/train data/local/train"
  exit 1; 
fi 
...
for f in spk2utt utt2spk wav.scp text segments reco2file_and_channel; do
  cp $dir/$f $datadir/$f || exit 1;
done

if [ $# == 2 ]; then # fix speaker IDs
  find $2 -name conv.tab > $dir/conv.tab
  local/swbd1_fix_speakerid.pl `cat $dir/conv.tab` $datadir
  utils/utt2spk_to_spk2utt.pl $datadir/utt2spk.new > $datadir/spk2utt.new
  # patch files
  for f in spk2utt utt2spk text segments; do
    cp $datadir/$f $datadir/$f.old || exit 1;
    cp $datadir/$f.new $datadir/$f || exit 1;
  done
  rm $dir/conv.tab
fi 
