...
gunzip -c $lmfile | \
  ...
  utils/s2eps.pl | \
  fstcompile --isymbols=$langdir/$words \
  --osymbols=$langdir/$words  --keep_isymbols=false --keep_osymbols=false | \
  fstrmepsilon > $destdir/$Gfst || exit 1
fstisstochastic $destdir/$Gfst || true;
