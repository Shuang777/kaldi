...
ARGS=`getopt -o "d:p:sbSruG:P:o:i:mzh" -l "dictionary:,prefix:,syllable,break_syllables,constrain_syllables,roman_phones,remove_underscores,max_grapheme_cluster:,max_phoneme_cluster:,output_model:,interpolate:,multiple_em_estimations,clean,help" -- "$@"`
...
#grep -v '^<' $BASEDICT | g2p/substitute_vbar.pl > $BASEDICT.preprocess
...
#sed -e 's# \. # #g' $BASEDICT > $PREFIX.g2p_corpus
cp $BASEDICT  $PREFIX.g2p_corpus
G2P_CORPUS=$PREFIX.g2p_corpus
s1_char_delim=""
...
if [ ! -z "$SYLLABLE" ]; then
  ...
  if [ ! -z "$BREAK_SYLLABLES" ]; then
    ...
    perl -n -e 'print;
                ...
                @g=split(/ /,$grapheme);
                @s=split(/ /,$syl);
                if ($#g==$#s) { for ($i=0;$i<=$#g;$i++) {print "$g[$i]\t$s[$i]\n";} }' $PREFIX.g2p_corpus.romanbreak > $PREFIX.g2p_corpus.withbreaks
  fi
fi
...
if [ ! -z "$IGNORE_UNPRONOUNCEABLE" ]; then
  g2p/g2p_remove_unpronounceable.pl $PREFIX.g2p_corpus > $PREFIX.g2p_corpus.pron
  G2P_CORPUS=$PREFIX.g2p_corpus.pron
fi

echo "G2P_CORPUS is $G2P_CORPUS"

phonetisaurus-align \
  --input=$G2P_CORPUS \
  --ofile=$PREFIX.g2p.aligned \
  --s1_char_delim=$s1_char_delim \
  --skip='.' \
  --seq1_del=false \
  --seq2_del=false \
...
if float_cond "$INTERP < 1.0"; then
  echo "Build a non-multigram lm to interpolate with the main lm"
  phonetisaurus-align \
    --input=$G2P_CORPUS \
    --ofile=$PREFIX.g2p.aligned_G1 \
    --s1_char_delim=$s1_char_delim \
    --skip='.' \
    --seq1_max=1 \
    --seq2_max=7 ;
