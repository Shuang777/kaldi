...
<< run
local/timit_data_prep.sh `pwd`/863/863

local/swbd1_prepare_dict.sh lexicon1_utf8.txt

utils/prepare_lang.sh data/local/dict "<unk>" data/local/lang data/lang

# Now train the language models

local/swbd1_train_lms.sh corpus_LM_utf8.txt data/local/dict/lexicon.txt data/local/lma
prune-lm --threshold=1e-7 data/local/lm/sw1.o3g.kn.gz /dev/stdout \
  | gzip -c > data/local/lm/sw1.o3g.pr1-7.kn.gz
utils/format_lm_sri.sh data/lang data/local/lm/sw1.o3g.kn.gz data/lang_test
...
steps/make_mfcc.sh --cmd "$decode_cmd" --nj 20 data/test exp/make_mfcc/test $mfccdir
steps/compute_cmvn_stats.sh data/test exp/make_mfcc/test $mfccdir
utils/fix_data_dir.sh data/test  # remove segments with problems
...
utils/subset_data_dir.sh --shortest data/train_nodev 10000 data/train_10k

## Starting basic training on MFCC features
steps/train_mono.sh --nj 10 --cmd "$train_cmd" \
  data/train_10k data/lang exp/mono 
...
steps/train_deltas.sh --cmd "$train_cmd" \
  3200 30000 data/train data/lang exp/tri1_ali exp/tri2 
...
steps/train_lda_mllt.sh --cmd "$train_cmd" \
  5500 90000 data/train data/lang exp/tri2_ali exp/tri3b 
...
steps/train_sat.sh  --cmd "$train_cmd" \
  5500 90000 data/train data/lang exp/tri3b_ali \
   exp/tri4a 
...
steps/train_sat.sh  --cmd "$train_cmd" \
  11500 200000 data/train data/lang exp/tri3b_ali exp/tri4b

graph_dir=exp/tri4b/graph
$train_cmd $graph_dir/mkgraph.log \
  utils/mkgraph.sh data/lang_test exp/tri4b $graph_dir
steps/decode_fmllr.sh --nj 30 --cmd "$decode_cmd" --config conf/decode.config \
...
steps/make_denlats.sh --nj 50 --cmd "$decode_cmd" --config conf/decode.config \
  ...
  data/train data/lang exp/tri4b exp/tri4b_denlats 
