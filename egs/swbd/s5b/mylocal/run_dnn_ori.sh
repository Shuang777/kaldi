...
dbndir=exp/dnn5b_pretrain-dbn-ori
if [ $stage -le 1 ]; then
  # Pre-train DBN, i.e. a stack of RBMs
  (tail --pid=$$ -F $dbndir/log/pretrain_dbn.log 2>/dev/null)& # forward log
  $cuda_cmd $dbndir/log/pretrain_dbn.log \
    mysteps/nnet/pretrain_dbn.sh --rbm-iter 1 $data_fmllr/train_nodup $dbndir || exit 1;
fi

dnndir=exp/dnn5b_pretrain-dbn-ori_dnn
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dnndir/log/train_nnet.log \
    ...
    $data_fmllr/train_nodup_tr90 $data_fmllr/train_nodup_cv10 data/lang $ali $ali $dnndir || exit 1;
  # Decode (reuse HCLG graph)
  steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.08333 \
    $gmmdir/graph_sw1_fsh_tgpr $data_fmllr/eval2000 $dnndir/decode_eval2000_sw1_fsh_tgpr || exit 1;
...
#    $dnndir/decode_eval2000_sw1_fsh_tgpr $dnndir/decode_eval2000_sw1_fsh_tg.3 || exit 1 
fi
